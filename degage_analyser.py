...
degage_analyser.py
==================
Dedicated phase-detection + correction engine for Battement Dégagé.
Reads ballet_database.json for thresholds and bilingual cues.

Entry point: analyse_degage(pose_data)
"""
...
_TERM = next((t for t in _DB.get("terms", []) if t.get("id") == "battement_degage"), {})
_TERM_FR = _TERM.get("term_fr", "Battement Dégagé")
...
_MIN_FOOT_POINT = _CV.get("foot_extension", {}).get("min_offset_norm", 0.25)
...
def _presence_confidence(frames):
    ...
    meas = [f.get("measurements", {}) for f in frames]
    offs = [max(m.get("left_knee_toe_offset_norm", 0), m.get("right_knee_toe_offset_norm", 0)) for m in meas]
    if not offs: return 0.0
    baseline = sorted(offs)[int(len(offs) * 0.3)]
    peak = max(offs)
    extension = peak - baseline
    if extension < 0.28: return 0.0
...
TRIGGERS = [
    {
        "id": "degage_001",
        "severity": _DB_TRIGGERS.get("degage_001", {}).get("severity", "major"),
        ...
        "cue_pt": _DB_TRIGGERS.get("degage_001", {}).get("cue_pt",
            "Joelho de apoio a dobrar no dégagé. Estica completamente o joelho de suporte."),
        "cue_en": _DB_TRIGGERS.get("degage_001", {}).get("cue_en",
            "Supporting knee bending in dégagé. Fully extend the supporting knee."),
    },
    {
        "id": "degage_002",
        "severity": _DB_TRIGGERS.get("degage_002", {}).get("severity", "major"),
        "check": lambda m, phase: (
            phase in ("rising", "peak") and
            m.get("hip_height_diff_pct", 0) > _MAX_HIP_DIFF
        ),
        "cue_pt": _DB_TRIGGERS.get("degage_002", {}).get("cue_pt",
            "Quadril a subir com a perna. Mantém os quadris estabilizados."),
        "cue_en": _DB_TRIGGERS.get("degage_002", {}).get("cue_en",
            "Hip hiking with the leg. Keep hips stabilized."),
    },
    {
        "id": "degage_003",
        "severity": _DB_TRIGGERS.get("degage_003", {}).get("severity", "warning"),
        "check": lambda m, phase: (
            phase == "peak" and
            max(m.get("left_knee_toe_offset_norm", 0), m.get("right_knee_toe_offset_norm", 0)) < _MIN_FOOT_POINT
        ),
        "cue_pt": _DB_TRIGGERS.get("degage_003", {}).get("cue_pt",
            "Pé não completamente apontado. Estende através da ponta do pé."),
        "cue_en": _DB_TRIGGERS.get("degage_003", {}).get("cue_en",
            "Foot not fully pointed. Extend through the tip of the foot."),
    },
    {
        "id": "degage_004",
        "severity": _DB_TRIGGERS.get("degage_004", {}).get("severity", "warning"),
        "check": lambda m, phase: (
            phase in ("rising", "peak") and
            m.get("trunk_lean_angle", 0) > _MAX_LEAN
        ),
        "cue_pt": _DB_TRIGGERS.get("degage_004", {}).get("cue_pt",
            "Tronco instável no dégagé. Mantém o eixo fixo."),
        "cue_en": _DB_TRIGGERS.get("degage_004", {}).get("cue_en",
            "Trunk instability in dégagé. Keep the axis fixed."),
    },
]
...
HYSTERESIS_CONFIG = {
    "degage_001": (10.0, 6.0),
    "degage_002": (_MAX_HIP_DIFF, max(0.5, _MAX_HIP_DIFF - 1.0)),
    "degage_004": (_MAX_LEAN, max(1.0, _MAX_LEAN - 2.0)),
}

TRIGGER_EVIDENCE = {
    "degage_001": ("knee_angle", _MIN_KNEE),
    "degage_002": ("hip_height_diff_pct", _MAX_HIP_DIFF),
    "degage_003": ("knee_toe_offset_norm", _MIN_FOOT_POINT),
    "degage_004": ("trunk_lean_angle", _MAX_LEAN),
}


def _trigger_signal(trigger_id, m, phase):
    if trigger_id == "degage_001" and phase in ("rising", "peak", "descent"):
        return max(180 - m.get("left_knee_angle", 180), 180 - m.get("right_knee_angle", 180))
    if trigger_id == "degage_002" and phase in ("rising", "peak"):
        return m.get("hip_height_diff_pct", 0)
    if trigger_id == "degage_004" and phase in ("rising", "peak"):
...
def analyse_degage(pose_data):
    ...
    if not movement_detected:
        return {
            "exercise": _TERM_FR, "exercise_id": "battement_degage",
