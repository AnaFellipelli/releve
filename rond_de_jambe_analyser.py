"""
rond_de_jambe_analyser.py
=========================
Dedicated phase-detection + correction engine for Rond de Jambe à Terre.
Reads ballet_database.json for thresholds and bilingual cues.

Entry point: analyse_rond_de_jambe(pose_data)
"""
...
_TERM = next((t for t in _DB.get("terms", []) if t.get("id") == "rond_de_jambe"), {})
_TERM_FR = _TERM.get("term_fr", "Rond de Jambe à Terre")
_CV = _TERM.get("correct_values", {})
_DB_TRIGGERS = {t["id"]: t for t in _TERM.get("correction_triggers", []) if "id" in t}

_MAX_HIP_DIFF  = _CV.get("hip_squareness", {}).get("max_height_diff_pct", 2.5)
...
_MAX_ROT       = _CV.get("hip_rotation", {}).get("max_rotation_deg", 12)
...
TRIGGERS = [
    {
        "id": "rond_001",
        "severity": _DB_TRIGGERS.get("rond_001", {}).get("severity", "major"),
        "check": lambda m, phase: m.get("hip_height_diff_pct", 0) > _MAX_HIP_DIFF,
        "cue_pt": _DB_TRIGGERS.get("rond_001", {}).get("cue_pt",
            "Quadril a abrir no rond de jambe. Mantém os quadris quadrados à frente."),
        "cue_en": _DB_TRIGGERS.get("rond_001", {}).get("cue_en",
            "Hip squareness lost in rond de jambe. Keep hips square to the front."),
    },
    {
        "id": "rond_002",
        "severity": _DB_TRIGGERS.get("rond_002", {}).get("severity", "major"),
        "check": lambda m, phase: (
            m.get("left_knee_angle", 180) < _MIN_KNEE or
            m.get("right_knee_angle", 180) < _MIN_KNEE
        ),
        "cue_pt": _DB_TRIGGERS.get("rond_002", {}).get("cue_pt",
            "Joelho de apoio a dobrar. Mantém-no completamente esticado durante o círculo."),
        "cue_en": _DB_TRIGGERS.get("rond_002", {}).get("cue_en",
            "Supporting knee bending. Keep it fully extended throughout the circle."),
    },
    {
        "id": "rond_003",
        "severity": _DB_TRIGGERS.get("rond_003", {}).get("severity", "warning"),
        "check": lambda m, phase: m.get("trunk_lean_angle", 0) > _MAX_LEAN,
        "cue_pt": _DB_TRIGGERS.get("rond_003", {}).get("cue_pt",
            "Tronco a compensar no rond de jambe. Eixo estável enquanto a perna trabalha."),
        "cue_en": _DB_TRIGGERS.get("rond_003", {}).get("cue_en",
            "Trunk compensating in rond de jambe. Stable axis while the leg works."),
    },
    {
        "id": "rond_004",
        "severity": _DB_TRIGGERS.get("rond_004", {}).get("severity", "warning"),
        "check": lambda m, phase: m.get("hip_rotation_deg", 0) > _MAX_ROT,
        "cue_pt": _DB_TRIGGERS.get("rond_004", {}).get("cue_pt",
            "Quadril a rodar. Mantém o en dehors sem rodar o quadril."),
        "cue_en": _DB_TRIGGERS.get("rond_004", {}).get("cue_en",
            "Hip twisting. Maintain en dehors without rotating the hip."),
    },
]
...
HYSTERESIS_CONFIG = {
    "rond_001": (_MAX_HIP_DIFF, max(1.0, _MAX_HIP_DIFF - 1.0)),
    "rond_002": (7.0, 4.0),
    "rond_003": (_MAX_LEAN, max(3.0, _MAX_LEAN - 2.0)),
    "rond_004": (_MAX_ROT, max(6.0, _MAX_ROT - 4.0)),
}

TRIGGER_EVIDENCE = {
    "rond_001": ("hip_height_diff_pct", _MAX_HIP_DIFF),
    "rond_002": ("knee_angle", _MIN_KNEE),
    "rond_003": ("trunk_lean_angle", _MAX_LEAN),
    "rond_004": ("hip_rotation_deg", _MAX_ROT),
}
...
def analyse_rond_de_jambe(pose_data):
    ...
    if not movement_detected:
        return {
            "exercise": _TERM_FR, "exercise_id": "rond_de_jambe",
